...
export const CalorieResults = ({ data }: CalorieResultsProps) => {
  return (
    <Card className="border-0 shadow-xl bg-gradient-to-br from-purple-50 to-pink-50 animate-fade-in">
      <CardHeader>
      ...
      <CardContent className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <p className="text-lg font-semibold capitalize">{data.dishName}</p>
          </div>
        ...
        <div className="bg-white/60 rounded-lg p-4 space-y-4">
          ...
          {data.servings > 1 && (
            <div className="text-center text-sm text-gray-600 pt-2 border-t border-gray-200">
              {data.caloriesPerServing} × {data.servings} = {data.totalCalories}{" "}
              calories
            </div>
          )}
        </div>
